import tensorflow as tf
import numpy as np

file1 = "./data/CIFAR10_data/cifar-10-batches-py/data_batch_1"
file2  = "./data/CIFAR10_data/cifar-10-batches-py/data_batch_1"
file3  = "./data/CIFAR10_data/cifar-10-batches-py/data_batch_1"
file4  = "./data/CIFAR10_data/cifar-10-batches-py/data_batch_1"
file5  = "./data/CIFAR10_data/cifar-10-batches-py/data_batch_1"
test_file = "./data/CIFAR10_data/cifar-10-batches-py/test_batch"

def unpickle(file):
    import pickle
    with open(file, 'rb') as fo:
        dict = pickle.load(fo, encoding='bytes')
    return dict

def labels_one_hot(label):
    np_array = np.array(label)
    return (np.arange(10) == np_array[:,None]).astype(np.float32)

def combine_batches_data(data_type = "train"):
    if data_type == "train":
        b1,b2,b3,b4,b5 = unpickle(file1),unpickle(file2),unpickle(file3),unpickle(file4),unpickle(file5)
        train_img = np.concatenate((b1[b'data'],b2[b'data'],b3[b'data'],b4[b'data'],b5[b'data']))
        train_label = np.concatenate((b1[b'labels'],b2[b'labels'],b3[b'labels'],b4[b'labels'],b5[b'labels']))
        train_labelname = np.concatenate((b1[b'filenames'],b2[b'filenames'],b3[b'filenames'],b4[b'filenames'],b5[b'filenames']))
    elif data_type == "test":
        test = unpickle(test_file)
        train_img = test[b'data']
        train_label = test[b'labels']
        train_labelname = test[b'filenames']
    label_one_hot=labels_one_hot(train_label)
    return train_img,label_one_hot,train_labelname

def next_batch(num, data, labels):
    idx = np.arange(0 , len(data))
    np.random.shuffle(idx)
    idx = idx[:num]
    data_shuffle = [data[ i] for i in idx]
    labels_shuffle = [labels[ i] for i in idx]
    return np.asarray(data_shuffle), np.asarray(labels_shuffle)

def weight_variable(shape):
    with tf.name_scope("Weights"):
        initial = tf.truncated_normal(shape, stddev=0.1,name="Weights")
    return tf.Variable(initial)

def bias_variable(shape):
    with tf.name_scope("biases"):
        initial = tf.constant(0.1, shape=shape,name="biases")
    return tf.Variable(initial)

def conv2d(x, W):
    # stride [1, x_movement, y_movement, 1]
    # Must have strides[0] = strides[3] = 1
    return tf.nn.conv2d(x, W, strides=[1, 1, 1, 1], padding='SAME')

def max_pool_2x2(x):
    # stride [1, x_movement, y_movement, 1]
    return tf.nn.max_pool(x, ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME')

train_img,train_label,train_labelname = combine_batches_data()
test_img,test_label,test_labelname = combine_batches_data("test")


# define placeholder for inputs to network
with tf.name_scope("CIFAR_Dataset"):
    xs = tf.placeholder(tf.float32, [None,3072],name = "x_data")
    x_image = tf.reshape(xs, [-1, 32, 32, 3])
    ys = tf.placeholder(tf.float32,[None,10],name = "y_data")

keep_prob = tf.placeholder(tf.float32,name="Dropout")


# print(x_image.shape)  # [n_samples, 28,28,1]

with tf.name_scope("ConvNN_Layer1"):
    W_conv1 = weight_variable([3,3,3,32]) # patch 5x5, in size 1, out size 32
    b_conv1 = bias_variable([32])
    h_conv1 = tf.nn.relu(conv2d(x_image, W_conv1) + b_conv1) # output size 32*32*32
with tf.name_scope("Max_Pooling"):
    h_pool1 = max_pool_2x2(h_conv1)   
#with tf.name_scope("Dropout"):
#    dropout1 = tf.nn.dropout(h_pool1,keep_prob) #16*16*32
    
with tf.name_scope("ConvNN_Layer2"):
    W_conv2 = weight_variable([3,3, 32, 64]) # patch 5x5, in size 32, out size 64
    b_conv2 = bias_variable([64])
    h_conv2 = tf.nn.relu(conv2d(h_pool1, W_conv2) + b_conv2) # output size 14x14x64
with tf.name_scope("Max_Pooling"):
    h_pool2 = max_pool_2x2(h_conv2)
#with tf.name_scope("Dropout"):
#    dropout2 = tf.nn.dropout(h_pool2,keep_prob)        #8*8*64

with tf.name_scope("ConvNN_Layer3"):
    W_conv3 = weight_variable([3,3, 64, 128]) # patch 5x5, in size 32, out size 64
    b_conv3 = bias_variable([128])
    h_conv3 = tf.nn.relu(conv2d(h_pool2, W_conv3) + b_conv3) # output size 14x14x64
with tf.name_scope("Max_Pooling"):
    h_pool3 = max_pool_2x2(h_conv3)   #4*4*128

with tf.name_scope("Full_Connection_Layer1"):
    W_fc1 = weight_variable([4*4*128, 625])
    b_fc1 = bias_variable([625])
    # [n_samples, 7, 7, 64] ->> [n_samples, 7*7*64]
    h_pool3_flat = tf.reshape(h_pool3, [-1, 4*4*128])
    h_fc1 = tf.nn.relu(tf.matmul(h_pool3_flat, W_fc1) + b_fc1)
    with tf.name_scope("Dropout"):
        h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob)  


with tf.name_scope("Full_Connection_Layer2"):
    W_fc2 = weight_variable([625, 10])
    b_fc2 = bias_variable([10])
    prediction = tf.nn.softmax(tf.matmul(h_fc1_drop, W_fc2) + b_fc2, name="Predict_y_data")


# the error between prediction and real data
with tf.name_scope("Cross_Entropy"):
    cross_entropy = tf.reduce_mean(-tf.reduce_sum(ys * tf.log(prediction),reduction_indices=[1]))
tf.summary.scalar("Cross_Entorpy",cross_entropy)
with tf.name_scope("Train"):
    train = tf.train.AdamOptimizer(0.0001).minimize(cross_entropy)
with tf.name_scope("Accuracy"):
    similiar_y_level = tf.equal(tf.arg_max(prediction,1),tf.arg_max(ys,1))
    accuracy = tf.reduce_mean(tf.cast(similiar_y_level,tf.float32))
tf.summary.scalar("Accuracy/MNIST_Data",accuracy)


""" 準備餵資料給框架 """
def feed_dict(feed_type = "train"):
    if feed_type == "train":
        batch_x,batch_y = next_batch(100,train_img,train_label)
        dropout = 0.6
        return {xs:batch_x,ys:batch_y,keep_prob:dropout}
    elif feed_type == "test":
        x_test_data,y_test_data = test_img[:1000],test_label[:1000]
        dropout = 1.0
        return {xs:x_test_data,ys:y_test_data,keep_prob:dropout}

with tf.Session() as sess:
    merged = tf.summary.merge_all()
    train_writer = tf.summary.FileWriter("C:/board/train",sess.graph)
    test_writer = tf.summary.FileWriter("C:/board/test",sess.graph)
    tf.global_variables_initializer().run()
    for i in range(1001):
        summary,_ = sess.run([merged,train],feed_dict=feed_dict("train"))
        train_writer.add_summary(summary,i)
        if i % 50 == 0:
            summary_,acc = sess.run([merged,accuracy],feed_dict=feed_dict("test"))
            test_writer.add_summary(summary_,i)
            print("Acc",i//50,": ",acc)       
 
